	...
	<div class="admin-guides-page">
		<header class="page-header">
			<div class="header-text">
				<h1>Manage Guides</h1>
				<p class="subtitle">Create, monitor, and oversee all curated knowledge bases.</p>
			</div>

			<router-link to="/admin/guides/create">
				<button class="btn-primary"><i class="fa-solid fa-plus"></i> Create New Guide</button>
			</router-link>
		</header>
		...
		<div v-else-if="guides.length === 0" class="state-container state-empty">
			...
			<router-link to="/admin/guides/create" style="margin-top: 16px; display: inline-block">
				<button class="btn-primary">Write Your First Guide</button>
			</router-link>
		</div>

		<div v-else class="guide-grid">
			<GuideCard v-for="guide in guides" :key="guide.id" :guide="guide" :isAdmin="true" @delete="deleteGuide" />
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		async deleteGuide(id) {
			if (!confirm("Are you sure you want to delete this guide completely?")) return;
			const res = await apiDeleteGuide(id);
			if (!res.success) return this.flash.error(res.message || "Deletion sequence failed.");
...
/* ── Grid Layout (Identical to Spots Layout) ── */
.guide-grid {
	display: grid;
